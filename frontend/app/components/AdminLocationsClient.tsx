      ...
      <section className="relative w-full overflow-hidden">
        ...
        <div className="relative w-full px-4 sm:px-6 lg:px-12 py-10 md:py-14">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            ...
            <button onClick={handleAdd} className="inline-flex items-center gap-2 px-6 py-3 bg-[#E8B830] text-[#1a0a10] text-sm font-semibold rounded-full hover:bg-[#E8B830] transition-colors shadow-lg shadow-[#E8B830]/25">
        ...
        <div className="w-full px-4 sm:px-6 lg:px-12">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            {locations.map((location) => (
              <div key={location.id} className="bg-gray-50 rounded-xl p-5 border border-[#1A4FAD]/10 hover:border-[#1A4FAD]/25 hover:shadow-md transition-all group">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-[#E8B830]/10 rounded-xl flex items-center justify-center"><MapPin className="w-5 h-5 text-[#E8B830]" /></div>
                    <h3 className="font-bold text-[#1A4FAD]">{location.name}</h3>
                  </div>
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => handleEdit(location)} className="p-1.5 text-gray-400 hover:text-[#E8B830] transition rounded-lg hover:bg-[#E8B830]/5"><Edit className="w-4 h-4" /></button>
                ...
                <div className="space-y-2.5 mb-4">
                  <p className="text-sm text-gray-600 flex items-start gap-2"><MapPin className="w-4 h-4 mt-0.5 shrink-0 text-[#E8B830]/50" />{location.address}</p>
                  <p className="text-sm text-gray-600 flex items-center gap-2"><Phone className="w-4 h-4 text-[#E8B830]/50" />{location.phone}</p>
                  <p className="text-sm text-gray-600 flex items-start gap-2"><Clock className="w-4 h-4 mt-0.5 shrink-0 text-[#E8B830]/50" />{location.hours}</p>
                </div>
                <div className="flex items-center justify-between pt-3 border-t border-gray-200">
                  ...
                  <button onClick={() => handleToggleActive(location)} className="text-xs text-[#1A4FAD] hover:text-[#E8B830] font-medium transition-colors">{location.is_active ? "Deactivate" : "Activate"}</button>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-[200] flex items-center justify-center p-4" onClick={() => setShowModal(false)}>
          <div className="bg-white rounded-2xl max-w-md w-full p-6 border border-[#1A4FAD]/15 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-5">
              <h2 className="text-xl font-bold text-[#1A4FAD]">{editingLocation ? "Edit Location" : "Add New Location"}</h2>
            ...
            <div className="space-y-4">
              <div><label className="block text-sm font-medium text-gray-700 mb-2">Location Name</label>
                <input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="w-full px-4 py-3 border-2 border-[#1A4FAD]/15 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#1A4FAD]/15 focus:border-[#1A4FAD]/40 text-sm" placeholder="e.g. North Legon" />
              </div>
              <div><label className="block text-sm font-medium text-gray-700 mb-2">Address</label>
                <input type="text" value={formData.address} onChange={(e) => setFormData({ ...formData, address: e.target.value })} className="w-full px-4 py-3 border-2 border-[#1A4FAD]/15 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#1A4FAD]/15 focus:border-[#1A4FAD]/40 text-sm" placeholder="Full address" />
              </div>
              <div><label className="block text-sm font-medium text-gray-700 mb-2">Phone</label>
                <input type="text" value={formData.phone} onChange={(e) => setFormData({ ...formData, phone: e.target.value })} className="w-full px-4 py-3 border-2 border-[#1A4FAD]/15 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#1A4FAD]/15 focus:border-[#1A4FAD]/40 text-sm" placeholder="e.g. 0559497906" />
              </div>
              <div><label className="block text-sm font-medium text-gray-700 mb-2">Hours</label>
                <textarea value={formData.hours} onChange={(e) => setFormData({ ...formData, hours: e.target.value })} rows={2} className="w-full px-4 py-3 border-2 border-[#1A4FAD]/15 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#1A4FAD]/15 focus:border-[#1A4FAD]/40 text-sm resize-none" placeholder="e.g. Mon-Sat: 8am-5pm" />
              </div>
              <div className="flex gap-3 pt-2">
                ...
                <button onClick={handleSave} className="flex-1 bg-[#1A4FAD] text-white px-4 py-3 rounded-xl font-semibold text-sm hover:bg-[#163E8A] transition flex items-center justify-center gap-2"><Save className="w-4 h-4" />{editingLocation ? "Save" : "Add"}</button>
              </div>
            </div>
          </div>
        </div>
