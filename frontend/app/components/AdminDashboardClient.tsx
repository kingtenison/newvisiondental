...
export default function AdminDashboardClient({
  ...
  stats: { totalAppointments: number; todayAppointments: number; pendingAppointments: number; totalPatients: number };
                  ...
                  {recentAppointments.length === 0 ? (
                    ...
                    recentAppointments.map((apt) => (
                      <tr key={apt.id} className="hover:bg-[#FFFDF5] transition">
                        ...
                        <td className="px-5 py-3.5"><span className={`px-2.5 py-1 rounded-full text-xs font-medium ${statusColors[apt.status]}`}>{apt.status?.charAt(0).toUpperCase() + apt.status?.slice(1)}</span></td>
                      </tr>
                    ))
                  )}
                </tbody>
