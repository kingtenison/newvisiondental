...
export function Header() {
  ...
  return (
    <>
                  ...
                  <div className="flex md:hidden items-center">
                    {!isLoading && (
                      session ? (
                        ...
                        <Link
                          href="/login"
                          className="px-4 py-2 text-white text-sm font-medium rounded-full bg-white/10 hover:bg-white/20 transition-all duration-300 border border-white/20"
                        >
                          Log in
                        </Link>
                      )
      ...
      {/* Mobile Bottom Navigation - YouTube/Android Style */}
          ...
          {mobileNavItems.map((item) => {
            ...
            if (isBookNow) {
              return (
                  <Link
                    key={item.href}
                    href={item.href}
                    className="flex flex-col items-center justify-center py-2 px-3 -mt-6"
                  >
                    <div className="w-14 h-14 rounded-full btn-golden-shine flex items-center justify-center shadow-lg shadow-[#D4AF37]/30 mb-1">
                      <Icon className="w-6 h-6" />
                    </div>
                  <span className="text-[10px] font-semibold text-[#D4AF37]">{item.label}</span>
                </Link>
              )
            }
            ...
            if (isCallItem) {
              return (
                <a
                  key={item.href}
                  href={item.href}
                  className="flex flex-col items-center justify-center py-2 px-3 text-[#5C0F22] hover:text-[#D4AF37] transition-colors duration-200"
                >
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className="flex flex-col items-center justify-center py-2 px-3 text-gray-500 hover:text-[#D4AF37] transition-colors duration-200"
              >
                <Icon className="w-6 h-6 mb-1" />
                <span className="text-[10px] font-medium">{item.label}</span>
              </Link>
