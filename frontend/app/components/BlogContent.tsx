        ...
        <div className="relative w-full px-4 sm:px-6 lg:px-12 flex-1 flex items-center justify-center py-10 md:py-14">
          <div className="text-center max-w-3xl mx-auto animate-[fadeInUp_0.8s_ease-out]">
            ...
            <p className="text-white/70 text-lg md:text-xl leading-relaxed">
              Expert insights, tips, and advice for a healthier smile.
              Stay informed with the latest in dental care.
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="w-full px-4 sm:px-6 lg:px-12">
          {filteredPosts.length === 0 ? (
            ...
            <>
                    ...
                    <div className="group relative bg-white rounded-3xl border border-[#E8B830]/15 overflow-hidden hover:shadow-xl hover:shadow-[#E8B830]/8 hover:border-[#E8B830]/30 transition-all duration-500">
                      <div className="grid md:grid-cols-2 gap-0">
                        ...
                        <div className="p-8 md:p-10 flex flex-col justify-center">
                          ...
                          <h2 className="text-2xl sm:text-3xl font-bold text-[#1A4FAD] mb-4 group-hover:text-[#E8B830] transition-colors duration-300 leading-tight">
                            {featuredPost.title}
                          </h2>
                          ...
                          <div className="flex items-center gap-5 text-xs text-gray-500 mb-6">
                            <div className="flex items-center gap-1.5">
                              <User className="w-3.5 h-3.5 text-[#E8B830]/60" />
                              <span>{featuredPost.author?.split(' ')[0] || 'Admin'}</span>
                            </div>
                            <div className="flex items-center gap-1.5">
                              <Calendar className="w-3.5 h-3.5 text-[#E8B830]/60" />
                              <span>{new Date(featuredPost.published_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span>
                            </div>
                            {featuredPost.read_time && (
                              <div className="flex items-center gap-1.5">
                                <Clock className="w-3.5 h-3.5 text-[#E8B830]/60" />
                                <span>{featuredPost.read_time}</span>
                              </div>
                            )}
                          </div>
                          <div className="inline-flex items-center gap-2 text-[#1A4FAD] font-semibold text-sm group-hover:gap-3 transition-all">
              ...
              {/* Remaining Articles Grid — CSS transitions instead of framer-motion */}
                        ...
                        <div className="group bg-white rounded-2xl border border-[#E8B830]/10 overflow-hidden hover:shadow-lg hover:shadow-[#E8B830]/6 hover:border-[#E8B830]/25 transition-all duration-300 h-full flex flex-col">
                          ...
                          <div className="relative h-56 w-full overflow-hidden">
                            {post.featured_image ? (
                              ...
                              <div className="w-full h-full bg-gradient-to-br from-[#1A4FAD]/5 via-[#E8B830]/5 to-[#1A4FAD]/5 flex items-center justify-center">
                                <BookOpen className="w-10 h-10 text-[#E8B830]/20" />
                              </div>
                            )}
                            <div className="absolute inset-0 bg-gradient-to-t from-black/40 via-black/10 to-transparent" />
                            {post.read_time && (
                              <div className="absolute top-3 right-3 flex items-center gap-1 bg-white/90 backdrop-blur-sm text-[#1A4FAD] text-[10px] font-semibold px-2.5 py-1 rounded-full">
                            ...
                            {post.tags && post.tags.length > 0 && (
                              <div className="absolute bottom-3 left-3 flex flex-wrap gap-1.5">
                                {post.tags.slice(0, 2).map((tag: string) => (
                                  <span key={tag} className="text-[10px] px-2 py-0.5 bg-white/90 backdrop-blur-sm text-[#1A4FAD] rounded-full font-medium">{tag}</span>
                                ))}
                              </div>
                            )}
                          </div>
                          ...
                          <div className="p-6 flex-1 flex flex-col">
                            <div className="flex items-center gap-3 text-[11px] text-gray-500 mb-3">
                              <div className="flex items-center gap-1">
                                <User className="w-3 h-3 text-[#E8B830]/60" />
                                <span>{post.author?.split(' ')[0] || 'Admin'}</span>
                              </div>
                              <span className="text-gray-300">|</span>
                              <div className="flex items-center gap-1">
                                <Calendar className="w-3 h-3 text-[#E8B830]/60" />
                            ...
                            <div className="bg-gray-50 rounded-lg p-3 mb-4 border-l-2 border-[#E8B830]/30">
                              <p className="text-gray-500 text-xs leading-relaxed line-clamp-3 italic">
                                {post.excerpt ? post.excerpt.substring(0, 180) + (post.excerpt.length > 180 ? '...' : '') : 'Click to read the full article.'}
                              </p>
                            </div>
                            <div className="mt-auto flex items-center justify-between pt-3 border-t border-gray-100">
                              <div className="flex items-center gap-1.5">
                                <BookOpen className="w-3.5 h-3.5 text-[#E8B830]/50" />
                                <span className="text-xs text-gray-500">{post.read_time || '5 min read'}</span>
                              </div>
                              <span className="inline-flex items-center gap-1 text-[#1A4FAD] text-xs font-semibold group-hover:gap-2 transition-all">
