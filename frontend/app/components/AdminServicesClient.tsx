        ...
        <div className="w-full px-4 sm:px-6 lg:px-12">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {services.map((service) => (
              <div key={service.id} className="bg-gray-50 rounded-xl p-5 border border-[#1A4FAD]/10 hover:border-[#1A4FAD]/25 hover:shadow-md transition-all group">
                <div className="flex items-start justify-between mb-3">
                  <h3 className="font-bold text-[#1A4FAD]">{service.name}</h3>
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => handleEdit(service)} className="p-1.5 text-gray-400 hover:text-[#E8B830] transition rounded-lg hover:bg-[#E8B830]/5"><Edit className="w-4 h-4" /></button>
                ...
                <div className="flex items-center gap-2 text-sm text-gray-600 mb-4"><Clock className="w-4 h-4 text-[#E8B830]/60" />{service.duration}</div>
                <div className="flex items-center justify-between pt-3 border-t border-gray-200">
                  ...
                  <button onClick={() => handleToggleActive(service)} className="text-xs text-[#1A4FAD] hover:text-[#E8B830] font-medium transition-colors">{service.is_active ? "Deactivate" : "Activate"}</button>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-[200] flex items-center justify-center p-4" onClick={() => setShowModal(false)}>
          <div className="bg-white rounded-2xl max-w-md w-full p-6 border border-[#1A4FAD]/15 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-5">
              <h2 className="text-xl font-bold text-[#1A4FAD]">{editingService ? "Edit Service" : "Add New Service"}</h2>
            ...
            <div className="space-y-4">
              <div><label className="block text-sm font-medium text-gray-700 mb-2">Service Name</label>
                <input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="w-full px-4 py-3 border-2 border-[#1A4FAD]/15 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#1A4FAD]/15 focus:border-[#1A4FAD]/40 text-sm" placeholder="e.g. Teeth Whitening" />
              </div>
              <div><label className="block text-sm font-medium text-gray-700 mb-2">Duration</label>
                <input type="text" value={formData.duration} onChange={(e) => setFormData({ ...formData, duration: e.target.value })} className="w-full px-4 py-3 border-2 border-[#1A4FAD]/15 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#1A4FAD]/15 focus:border-[#1A4FAD]/40 text-sm" placeholder="e.g. 60 mins" />
              </div>
              <div className="flex gap-3 pt-2">
                ...
                <button onClick={handleSave} className="flex-1 bg-[#1A4FAD] text-white px-4 py-3 rounded-xl font-semibold text-sm hover:bg-[#163E8A] transition flex items-center justify-center gap-2"><Save className="w-4 h-4" />{editingService ? "Save" : "Add"}</button>
              </div>
            </div>
          </div>
        </div>
