    ...
    <div className="min-h-screen bg-[#F2F4F8] pt-[72px] md:pt-20 overflow-x-hidden">
      ...
      <section className="relative w-full overflow-hidden">
        ...
        <div className="relative w-full px-4 sm:px-6 lg:px-12 py-12 md:py-16">
          {/* Back link */}
          <Link href="/blog" className="inline-flex items-center gap-2 text-white/60 hover:text-[#E8B830] text-sm font-medium mb-6 transition-colors group">
          ...
          <div className="flex flex-wrap items-center gap-5 text-white/60 text-sm">
            ...
            <div className="flex items-center gap-2">
              <Calendar className="w-4 h-4 text-[#E8B830]/60" />
              <span>{new Date(post.published_at).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</span>
            </div>
            {post.read_time && (
              <div className="flex items-center gap-2">
                <Clock className="w-4 h-4 text-[#E8B830]/60" />
                <span>{post.read_time}</span>
              </div>
            )}
            {post.views > 0 && (
              <div className="flex items-center gap-2">
                <Eye className="w-4 h-4 text-[#E8B830]/60" />
        ...
        <div className="w-full px-4 sm:px-6 lg:px-12">
          <div className="grid lg:grid-cols-4 gap-12">
            ...
            <div className="lg:col-span-3">
              ...
              {/* Share Section */}
              <div className="mt-12 pt-8 border-t border-[#E8B830]/15">
            ...
            <div className="lg:col-span-1">
              <div className="sticky top-24 space-y-6">
                {/* Quick Info Card */}
                <div className="bg-gray-50 rounded-2xl border border-[#E8B830]/15 p-6">
                  <h3 className="text-sm font-bold text-[#1A4FAD] uppercase tracking-wider mb-4">Article Info</h3>
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <span className="text-gray-500 text-sm">Author</span>
                      <span className="text-[#1A4FAD] text-sm font-semibold">{post.author?.split(' ')[0] || 'Admin'}</span>
                    </div>
                    <div className="h-px bg-gray-200" />
                    <div className="flex items-center justify-between">
                      <span className="text-gray-500 text-sm">Read Time</span>
                      <span className="text-[#1A4FAD] text-sm font-semibold">{post.read_time || '5 min'}</span>
                    </div>
                    <div className="h-px bg-gray-200" />
                    <div className="flex items-center justify-between">
                      <span className="text-gray-500 text-sm">Published</span>
                      <span className="text-[#1A4FAD] text-sm font-semibold">{new Date(post.published_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span>
                    </div>
                  </div>
                </div>

                {/* CTA Card */}
                <div className="bg-gradient-to-br from-[#1A4FAD] to-[#1A4FAD] rounded-2xl p-6 border border-[#E8B830]/15">
                ...
                {post.tags && post.tags.length > 0 && (
                  <div className="bg-gray-50 rounded-2xl border border-[#E8B830]/15 p-6">
                    <h3 className="text-sm font-bold text-[#1A4FAD] uppercase tracking-wider mb-4">Tags</h3>
                    <div className="flex flex-wrap gap-2">
                      {post.tags.map((tag: string) => (
                        <span key={tag} className="text-xs px-3 py-1.5 bg-[#E8B830]/8 text-[#1A4FAD] rounded-full border border-[#E8B830]/10 font-medium">
                          {tag}
                        </span>
                      ))}
                    </div>
        ...
        <div className="relative w-full px-4 sm:px-6 lg:px-12 text-center">
          <div className="max-w-2xl mx-auto">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <Link href="/blog" className="inline-flex items-center gap-2 px-8 py-4 rounded-full font-semibold btn-golden-shine shadow-lg shadow-[#E8B830]/25 hover:shadow-xl hover:shadow-[#E8B830]/40 transition-all duration-300 hover:scale-105">
